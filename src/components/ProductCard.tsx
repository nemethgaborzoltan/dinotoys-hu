...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div className="product-media">
      ...
      <button className={`heart ${liked ? 'selected' : ''}`} onClick={() => { shop.toggleWishlist(product.id, product); trackCommerce('add_to_wishlist', product) }} aria-label="Kedvencek">{liked ? '♥' : '♡'}</button>
    </div>
    <div className="product-body">
      ...
      <div className="card-actions"><button className="btn btn-primary" onClick={() => { shop.addToCart(product.id, 1, product); trackCommerce('add_to_cart', product) }}>Kosárba</button><button className={`btn btn-icon ${compared ? 'active' : ''}`} onClick={() => shop.toggleCompare(product.id, product)} title="Összehasonlítás">⇄</button></div>
    </div>
  </article>
}
