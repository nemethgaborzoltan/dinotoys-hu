...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div className="product-media">
      ...
      <button className={`heart ${liked ? 'selected' : ''}`} onClick={() => shop.toggleWishlist(product.id)} aria-label="Kedvencek">{liked ? '♥' : '♡'}</button>
    </div>
    <div className="product-body">
      ...
      <div className="card-actions"><button className="btn btn-primary" onClick={() => shop.addToCart(product.id)}>Kosárba</button><button className={`btn btn-icon ${compared ? 'active' : ''}`} onClick={() => shop.toggleCompare(product.id)} title="Összehasonlítás">⇄</button></div>
    </div>
  </article>
}
