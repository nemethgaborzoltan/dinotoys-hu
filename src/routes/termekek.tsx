...
function Products(){
  ...
  const filtered=useMemo(()=>{
    let result=products.filter((p)=>{
      ...
      if(search.q&&!(p.name+' '+p.brand+' '+p.tags.join(' ')).toLowerCase().includes(search.q.toLowerCase()))return false
      return true
    })
  ...
  return <div className="container section catalog-page"><div className="catalog-title"><div><span className="eyebrow">DinoToys katalógus</span><h1>{search.category||'Minden játék'}</h1><p>{filtered.length} termék</p></div><div className="catalog-actions"><select value={search.sort||''} onChange={(e)=>set({sort:e.target.value||undefined})}><option value="">Ajánlott sorrend</option><option value="trending">Trend</option><option value="rating">Legjobbra értékelt</option><option value="price-asc">Ár: növekvő</option><option value="price-desc">Ár: csökkenő</option></select></div></div><div className="catalog-layout"><aside className="filters"><div className="filter-block"><b>Keresés</b><input value={search.q||''} onChange={(e)=>set({q:e.target.value||undefined})}/></div><div className="filter-block"><b>Kategória</b><button className={!search.category?'selected':''} onClick={()=>set({category:undefined})}>Mind</button>{categories.map((c)=><button key={c.name} className={search.category===c.name?'selected':''} onClick={()=>set({category:c.name})}>{c.icon} {c.name}</button>)}</div><div className="filter-block"><b>Életkor</b><div className="filter-chips">{[3,4,6,8,10].map((age)=><button key={age} className={search.age===age?'selected':''} onClick={()=>set({age:search.age===age?undefined:age})}>{age}+ év</button>)}</div></div><div className="filter-block"><b>Maximum ár</b><div className="filter-chips">{[3000,5000,8000,10000].map((max)=><button key={max} className={search.max===max?'selected':''} onClick={()=>set({max:search.max===max?undefined:max})}>{max/1000}k</button>)}</div></div></aside><div>{filtered.length?<div className="product-grid catalog-grid">{filtered.map((p)=><ProductCard key={p.id} product={p}/>)}</div>:<div className="empty-state large"><span>🧩</span><h3>Nincs találat</h3><button className="btn btn-primary" onClick={()=>navigate({search:{}})}>Szűrők törlése</button></div>}</div></div></div>
}
